...
class TestFetchJson:
    ...
    def test_fetch_json_success(self, mock_get):
        ...
        mock_get.assert_called_once_with(
            "https://pokeapi.co/api/v2/pokemon/1", timeout=12
        )
...
class TestGetOrCreatePokemon:
    ...
    def test_create_new_pokemon(
        self, mock_build, mock_evolution, mock_load, db_session
    ):
...
class TestListStarters:
    ...
    def test_list_starters(self, mock_get_or_create, db_session):
        mock_get_or_create.side_effect = lambda db, name: Pokemon(
            name=name, pokeapi_id=1
        )
...
class TestGainExperience:
    ...
    def test_gain_experience_invalid_amount(self, db_session):
        trainer = User(
            username="trainer", email="trainer@test.com", hashed_password="hash"
        )
